...
class Student:
    ...
    # This part is image labels setting start 
        ...
        dep_combo["values"]=("Select Department","CSE","BBA","Ag","BCA","MCA","MBA")
        dep_combo.current(0)
        ...
        cou_combo["values"]=("Select Course Code","CCE323","EEE321","CIT321","CIT323","CCE321","CIT320")
        cou_combo.current(0)
        ...
        save_btn.grid(row=0,column=0,padx=5,pady=10,sticky=W)#7

        #update button
        update_btn=Button(btn_frame,command=self.update_data,text="Update",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        update_btn.grid(row=0,column=1,padx=5,pady=8,sticky=W)#7

        #delete button
        del_btn=Button(btn_frame,command=self.delete_data,text="Delete",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        del_btn.grid(row=0,column=2,padx=5,pady=10,sticky=W)#7

        #reset button
        reset_btn=Button(btn_frame,command=self.reset_data,text="Reset",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        reset_btn.grid(row=0,column=3,padx=5,pady=10,sticky=W)#7

        #take photo button
        take_photo_btn=Button(btn_frame,command=self.generate_dataset,text="Take Pic",width=12,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        ...
        #create table 
        self.student_table = ttk.Treeview(table_frame,column=("ID","Dep","Course","Year", "Sem","Name","Roll-No","Gender","DOB","Email","Mob-No","Address","Photo"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
...
# ==================Function Decleration==============================
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="":
            ...
            try:
                ...
                self.var_std_id.get(), #va_std_id.get()
                self.var_dep.get(), 
                self.var_course.get(),
                self.var_year.get(),
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Success","All Records has been Saved successfully!",parent=self.root)
    ...
    def get_cursor(self,event=""):
        ...
        self.var_dep.set(data[1]), #1
        self.var_course.set(data[2]), #2
        self.var_year.set(data[3]), #3
        self.var_semester.set(data[4]), #4
        #self.var_div.set(data[6]),
        self.var_gender.set(data[7]), #8
        self.var_dob.set(data[8]), #9
        self.var_mob.set(data[10]), #11
        self.var_address.set(data[11]),#12
        self.var_roll.set(data[6]), #7
        self.var_email.set(data[9]), #10
        ...
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(username='root', password='root',host='localhost',database='face_recognition',port=3306)
                    mycursor = conn.cursor()
                    mycursor.execute("update student set Name=%s,Dep=%s,course=%s,Year=%s,Semester=%s,Gender=%s,Dob=%s,Phone=%s,Address=%s,Roll=%s,Email=%s,PhotoSample=%s where Student_id=%s",(
                    self.var_std_name.get(),
                    self.var_dep.get(),
                    self.var_course.get(),
                    self.var_year.get(),
    ...
    def search_data(self):
        if self.var_search.get()=="" or self.var_searchTX.get()=="Select":
            ...
            try:
                ...
                sql = "SELECT Student_id,Name,Dep,course,Year,Semester,Gender,Dob,Phone,Address,Roll,Email,PhotoSample FROM student where Roll='" +str(self.var_search.get()) + "'"
